...
def get_handler(logger: Union[str, logging.Logger], handler_cls: logging.Handler) -> Optional[logging.Handler]:
...
def has_handler(logger: Union[str, logging.Logger], handler_cls: logging.Handler) -> bool:
...
def test_setup_with_default_config():
    ...
    np_logging.setup()
    assert has_handler('web', logging.handlers.SocketHandler)
    assert has_handler('email', logging.handlers.SMTPHandler)
    ...
    assert all(hasattr(log_record, attr) for attr in ('project', 'rig_name', 'comp_id', 'version'))
    
@pytest.fixture
def custom_handler_config() -> Dict:
    return np_config.fetch_file_config((pathlib.Path(__file__).parent / 'custom_handler_config.yaml').resolve())
...
def test_custom_handlers(custom_handler_config):
    ...
    handler_names = {handler: config.get('()', None) for handler, config in custom_handler_config['handlers'].items()}
    handler_classes = {handler:eval(cls) for handler, cls in handler_names.items() if cls}
    for logger in custom_handler_config['loggers']:
        for handler in custom_handler_config['loggers'][logger]['handlers']:
            ...
            if custom_handler_config['loggers'][logger].get('level', None):
                assert logging.getLogger(logger).level == logging.getLevelName(custom_handler_config['loggers'][logger]['level'])
            if custom_handler_config['handlers'][handler].get('level', None):
                assert logging._handlers[handler].level == logging.getLevelName(custom_handler_config['handlers'][handler]['level'])
...
def test_web_standalone():
    ...
    web = np_logging.web()
    assert logging.getLogger("web") is web
    assert has_handler(web, expected_handler)
    assert get_handler(web, expected_handler).host == "eng-mindscope"
...
def test_email_standalone():
    ...
    assert logging.getLogger("email") is email
